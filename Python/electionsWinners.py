...
candidate, so there will be no winner. The second candidate can win if all the
remaining candidates vote for him (3 + 3 = 6 > 5). The third candidate can win
...
be the winner (the votes array will thus be [3, 4, 5, 3]). The last candidate
...
def solution(votes: list[int], missing_votes: int) -> int:
    max_votes = max(votes)
    if missing_votes == 0:
        return 1 if votes.count(max_votes) == 1 else 0
    candidates_with_chances = 0
    for current_votes in votes:
        if current_votes + missing_votes > max_votes:
            candidates_with_chances += 1
    return candidates_with_chances
