...
output should be solution(someTime, leavingTime) = "2016-08-24 11:20".
...
time is correct. The time is given in the format "YYYY-MM-DD HH:MM".

"""
from datetime import datetime


def solution(some_time_raw: str, leaving_time_raw: str) -> str:
    time_format = "%Y-%m-%d %H:%M"
    some_time = datetime.strptime(some_time_raw, time_format)
    leaving_time = datetime.strptime(leaving_time_raw, time_format)
    reversed_target_time = some_time - (leaving_time - some_time)

    return reversed_target_time.strftime(time_format)
...
def main():
    ...
    print(solution(some_time, leaving_time))
